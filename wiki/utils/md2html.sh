...
PANDOC_FLAG+=" --css=%WIKI_TOPDIR%/../media/css/style.css" # 指名css样式文件.
...
# PANDOC_FLAG+=" --include-before-body templates/header.html"
PANDOC_FLAG+=" --include-before-body temp_title.html"
# PANDOC_FLAG+=" --include-after-body templates/footer.html"
...
# pandoc转换 # 调用pandoc编译
###################################
pandoc ${PANDOC_FLAG} --from=markdown --to=html ${src} -o $dst
